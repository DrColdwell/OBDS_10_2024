...
#history fastqc  --nogroup --threads 8 --extract -o ./fastqc_commanline fastq/ERR1755082_*


#hisat2 --threads 12 \
# -x /project/shared/resources/1_linux/genomes/Mus_musculus/Ensembl/GRCm38/Sequence/HISAT/mm10 \
# -1 /project/monc0033/1_linux/rnaseq/fastq/ERR1755082_1.fastq.gz \
# -2 /project/monc0033/1_linux/rnaseq/fastq/ERR1755082_2.fastq.gz \
# --rna-strandness RF \
# --summary-file /project/monc0033/1_linux/rnaseq/mapped_reads/ERR175508.stats.txt \
# -S /project/monc0033/1_linux/rnaseq/mapped_reads/ERR175508.sam

#template fastqc  --nogroup --threads 1 --extract -o /path/to/my_output_directory /path/to/my/sample_1.fastq.gz



#samtools view (add an option to include header)

# -@  meand the threads (alt --thread)  

samtools view -@ 12 -b -h  mapped_reads/ERR175508.sam > mapped_reads/ERR1755082.bam

# View 12 threads, -b output in .bam format, -h with a header, convert .sam to .bam

samtools sort -@ 12  mapped_reads/ERR1755082.bam  > mapped_reads/ERR1755082.sorted.bam 

# Sort with 12 threads

samtools index -@ 12 mapped_reads/ERR1755082.sorted.bam

# index  with 12 threads
#no output file for index
